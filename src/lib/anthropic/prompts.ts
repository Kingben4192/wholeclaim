...
//
// v2-golden-06 (2026-07-27): BRAND_VOICE.md addendum, Section B (AI Tool
// Prompt Binding) — added NEVER_LIST to policy/loss (Tier 2, soft
// binding, prompt-only) and decide/letter (Tier 1, hard binding, prompt
// plus the output-side filter in src/lib/anthropic/outputFilter.ts).
// gap (Estimate Gap Analyzer), mold, and supplement are untouched -- gap
// already carries its own guardrail line per the addendum; mold and
// supplement are not named in the addendum at all. Requires Golden Test
// Run 04 before Production status.
export const PROMPT_VERSION = "claim-binder-v2-golden-06";
...
// BRAND_VOICE.md addendum, Section B — shared verbatim across all four
// bound tools (Tier 1 and Tier 2 use the identical NEVER list; the only
// difference between tiers is whether the output-side filter also runs).
const NEVER_LIST =
  'NEVER: state or imply what a carrier owes, will pay, or is required to pay; recommend or imply the user should pursue litigation, legal action, or hire an attorney (a neutral "you may want to consult an attorney about this" is acceptable only when directly on-topic for a coverage question -- never proactive); draft language that reads as a formal legal demand, threat, or attorney correspondence; assert that a document, photo, or piece of evidence "proves," "confirms," or "establishes" fault, cause of damage, or liability; state or imply that following these recommendations increases the likelihood of approval, faster payment, or a larger settlement; cite insurance law, statutes, or legal standards as if giving legal advice; reference any specific carrier by name.';
