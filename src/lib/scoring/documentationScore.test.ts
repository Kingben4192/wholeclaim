...
describe("Evidence Quality & Organization", () => {
  ...
  // returned 1 instead of 0. Zero linked items (files.length === 0, so
  // linkedItems.length === 0 too) must score zero on BOTH halves --
  // labeledFraction was fixed in the same pass (Option 2, folded into
  // Decision #47) to also require at least one linked item before it can
  // award anything, closing the same class of vacuous-truth bug on the
  // labeling half.
  it("checking every item with zero files uploaded earns zero credit, not partial credit", () => {
    const result = computeDocumentationScore(
      baseInput({
        files: [],
        evidenceItems: [
    ...
    // No item has a file_id, so linkedItems is empty -- both linkedFraction
    // and labeledFraction must be 0 regardless of how well-written the
    // (unlinked) labels are.
    expect(result.categories.evidenceQualityOrganization.points).toBe(0);
    expect(result.categories.evidenceQualityOrganization.points).not.toBe(15);
  });

  it("scores full labeled credit when every linked item is well-labeled", () => {
    const result = computeDocumentationScore(
      baseInput({
        files: [
          { id: "f1", kind: "photo", original_name: "roof.jpg", uploaded_at: "2026-07-01" },
          { id: "f2", kind: "pdf", original_name: "policy.pdf", uploaded_at: "2026-07-01" },
        ],
        evidenceItems: [
          {
  ...
  // The core Option 2 regression: untouched-but-well-copywritten seeded
  // items must not dilute/mask a real mislabeled upload among the items
  // that actually have a file. Mirrors the 5-files/12-items example
  // audited 2026-07-26 (Decision #47 addendum): 5 linked items (4
  // well-labeled, 1 auto-generated), plus 7 untouched seeded items that
  // are well-labeled but were never uploaded against.
  it("does not let untouched seeded items mask a mislabeled linked upload", () => {
    const linkedWellLabeled = Array.from({ length: 4 }, (_, i) => ({
      label: `Real evidence description ${i}`,
      checked: true,
      file_id: `linked-${i}`,
      category: "evidence_coverage" as const,
      created_at: "2026-07-01",
    }));
    const linkedAutoGenerated = {
      label: "PDF — resume.pdf",
      checked: false,
      file_id: "linked-4",
      category: "documentation_completeness" as const,
      created_at: "2026-07-01",
    };
    const untouchedSeeded = Array.from({ length: 7 }, (_, i) => ({
      label: `Untouched template item ${i}`,
      checked: false,
      file_id: null,
      category: "documentation_completeness" as const,
      created_at: "2026-07-01",
    }));
    const files = [...linkedWellLabeled, linkedAutoGenerated].map((item, i) => ({
      id: item.file_id!,
      kind: "pdf" as const,
    ...
    const result = computeDocumentationScore(
      baseInput({
        files,
        evidenceItems: [...linkedWellLabeled, linkedAutoGenerated, ...untouchedSeeded],
      }),
      NOW,
    );
    // Old (pre-Option-2) formula: 11 of 12 total items pass the label check
    // (only the auto-generated one fails) -> labeledFraction = 11/12,
    // rounds the category up to 15/15. Correct formula: only the 5 LINKED
    // items count -> labeledFraction = 4/5 = 0.8 -> total 14/15.
    expect(result.categories.evidenceQualityOrganization.points).toBe(14);
    expect(result.categories.evidenceQualityOrganization.points).not.toBe(15);
  });
});
